...
//이미지
var images = "./images/worldcup_hsg/";
var imageList = [];//이미지 선로딩용
		...
		function loadImages(arr,callBack){
			...
			for (var i = 0; i < arr.length; i++) {
				...
				img.src = arr[i];
				document.getElementById("imagePreloader").innerHTML += "<img src='" + arr[i] + ".png' />";
...
function battle_ready(counter){
	...
	if (counter < 450) {
		...
		console.log($("#battle_img_right").style.right + "/" + $("#battle_img_right").offsetLeft);
		...
		//나머지 : 전투 실시 (클릭 대기)
		//1. 이미지 정지
		$("#battle_img_left").style.left = "15px";
		$("#battle_img_right").style.right = "15px";
		//2. 이미지선택 효과 활성화
...
window.onload = function() {
	...
	//이미지 선로딩 - 준비
	imageList.push("./images/worldcup_hsg/frame/mark_vs.png");
	imageList.push("./images/worldcup_hsg/frame/logo.png");
	...
	$("#loading_start").onclick = function() {
		...
		loadImages(imageList,function() {
			//아무 카드나 배치
			$("#intro_logo").src = images + chaList[Math.floor(Math.random() * chaList.length)][1] + ".png";
